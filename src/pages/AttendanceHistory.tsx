...
export default function AttendanceHistory() {
  ...
  const getWorkTypeColor = (workType: string) => {
    switch (workType) {
      case 'WFO': return 'bg-attendance-wfo text-white';
      case 'DL': return 'bg-attendance-dl text-white';
      case 'Cuti': return 'bg-attendance-cuti text-white';
      case 'Sakit': return 'bg-attendance-sakit text-white';
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      case 'Tepat Waktu': return 'text-success';
      case 'Terlambat': return 'text-warning';
      case 'Tidak Hadir': return 'text-destructive';
                    ...
                    >
                      <div className="flex-1">
                        <div className="flex items-center gap-2 mb-1">
                          ...
                          <div className={`px-2 py-1 rounded-md text-xs font-medium ${getWorkTypeColor(record.work_type)}`}>
                            {record.work_type}
                          </div>
                        </div>
                        <div className="flex items-center gap-4 text-sm text-muted-foreground">
                          <span>
                            Masuk: {record.clock_in_time ? format(parseISO(record.clock_in_time), 'HH:mm') : '--:--'}
                          </span>
                          <span>
                            Pulang: {record.clock_out_time ? format(parseISO(record.clock_out_time), 'HH:mm') : '--:--'}
                          </span>
                        </div>
